		...
		<div class="row">
			<table style="border-collapse: collapse;border: 0.52mm solid #4b6890;width:500px;">
				...
				<tr class="border-bottom">
					...
					<td colspan="3"><strong>{{ $info['record']->cus_lastname.', '.$info['record']->cus_firstname }}</strong></td>
				</tr>
				<tr>
					<td>Zone:</td>
					<td class="border-bottom">{{ strtoupper($info['record']->cus_zone) }}</td>
					<td>Meter No.:</td>
					<td class="text-center border-bottom">{{ $info['record']->meter_serial_no }}</td>
					<td>  </td>
				</tr>
				<tr class="border-bottom">
					<td>Month:</td>
					<td class="border-bottom">{{ date("M'y",strtotime($info['record']->reading_date)) }}</td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
				...
				<tr>
					...
					<td class="border-right border-bottom">
						@php
						if ($info['previous_record']) {
							$prev_date = date("d-M-y",strtotime($info['previous_record']->reading_date));
						}else{
							$prev_date = '';
						}
						@endphp
					...
					<td>PRESENT:</td>
					<td class="border-bottom">{{ (float)$info['record']->reading_prev_waterconsumed+(float)$info['record']->reading_waterconsumed }}</td>
					<td>cu.m</td>
				</tr>
				<tr>
					<td>TO:</td>
					<td class="border-right border-bottom">{{ date("d-M-y",strtotime($info['record']->reading_date)) }}</td>
					<td>PREVIOUS:</td>
					<td class="border-bottom">{{ $info['record']->reading_prev_waterconsumed }}</td>
					<td>cu.m</td>
				</tr>
				<tr class="border-bottom">
					<td>DUE DATE:</td>
					<td class="border-right border-bottom">{{ date("d-M",strtotime(get_due_date($info['record']->reading_date,$info['record']->custype_due_date_duration))) }}</td>
					<td>USAGE:</td>
					<td class="border-bottom">{{ $info['record']->reading_waterconsumed }}</td>
					<td>cu.m</td>
				</tr>
				<tr class="border-bottom">
				...
				<tr class="border-bottom">
					...
					<td class="border-bottom">{{ number_format($info['record']->reading_amount,2)}}</td>
				</tr>
				<tr>
					<td>Add:</td>
				...
				<tr>
					@php
						$total_amount = $info['record']->reading_amount + $info['record']->reading_other_payment + $info['arrears'];
					@endphp
				...
				<tr>
					@php
						$total_due_amount = calculate_penalty($info['record']->reading_amount,$info['record']->custype_due_date_penalty) + $info['record']->reading_other_payment + $info['arrears'];
					@endphp
